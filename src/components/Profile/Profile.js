      ...
      <div className="profile__container">
        ...
        <form className="profile__form" onSubmit={onEditSubmit}>
          <label className="profile__input-container">
            ...
            <input placeholder="Имя" className="profile__input" id="name" name="name" onChange={handleChange} value={name && values.name} required autoComplete="off" minLength="2" maxLength="40" />
          </label>
          <span className="profile__error" id='name-error'>{errors.name || ''}</span>
          <label className="profile__input-container">
            <span className="profile__input-title">E-mail</span>
            <input placeholder="E-mail" name="email" id="email" type="email" className="profile__input" pattern="^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$" onChange={handleChange} value={email && values.email} required minLength="2" maxLength="40" />
          </label>
